...
app.get("/task", validateToken, async (req, res) => {
  ...
  try {
    const tasks = status
      ? await Task.findAll({
          where: { status: { [Sequelize.Op.eq]: status }, userId },
        }) // Filtrar por userId
...
app.get("/task/details", validateToken, async (req, res) => {
...
// Actualizar un detalle de tarea
app.put("/task/id/details/:detailId", validateToken, async (req, res) => {
...
app.delete("/task/:id/history", validateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Todas las tareas del historial han sido eliminadas." });
  } catch (error) {
    console.error("Error al eliminar las tareas del historial:", error);
    res
      .status(500)
      .json({ error: "Error al eliminar las tareas del historial." });
  }
});

// Eliminar un detalle de tarea
router.delete("/task/id/details/:detailId", validateToken, async (req, res) => {
  const { taskId, detailId } = req.params;
